...
# Map prompt keys to your data fields
prompt_map = {
    'product': 'product',
    'sales rep': 'sales_rep',
    ...
    'expected revenue': 'revenue_from_deal'
}

# Extract values for each field in map
def extract_prompt_values(prompt, prompt_map):
    # Pattern: key value [comma/end]
    pattern = r'(?P<key>\w[\w ]+)\s+(?P<value>[^,]+)'
    found = dict(re.findall(pattern, prompt))
    result = {}
    for key, field in prompt_map.items():
        val = found.get(key)
        if val:
            # Try to convert field to number if numeric
            if field in ['sales_price', 'revenue_from_deal']:
                try:
                    val = float(val)
                except ValueError:
                    pass
...
def get_field_metrics(field, value):
    # Look up metrics (win rate, uplift, counts) for the field's value
    records = sales_metrics["categorical"].get(field, [])
    
    for rec in records:
        if rec.get(field) == value:
            result = {
                "win_rate": float(rec.get("Won", 0)),
                "uplift": float(rec.get("uplift", 0)),
...
def summarize_deal(deal):
    # Example: deal is a dict/object from vector search result
    summary = []
    if isinstance(deal, str):
        deal_dict = extract_prompt_values(deal, prompt_map)
    else:
        deal_dict = deal  # already a dic
    for field in ['product', 'account_sector', 'account_region','sales_price','revenue_from_deal']:
        val = deal_dict.get(field)
        if val:
            metrics = get_field_metrics(field, val)
            if metrics:
                summary.append(f"{field}: {val} | Win Rate: {metrics['win_rate']:.1%} | Uplift: {metrics['uplift']:.1%} | Count: {metrics['count']}")
    return '; '.join(summary)
...
def main():
    ...
    while True:
        ...
        ## Added as per Perplexity suggestions 
        user_fields = extract_prompt_values(prompt, prompt_map)
        user_metrics_summary = []
        for field in ['product', 'account_sector', 'account_region']:
