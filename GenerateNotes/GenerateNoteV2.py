...
def generate_notes(row):
    ...
    series = row['product_series'].lower()  # e.g., 'gtx', 'mg', 'gtk'
    product = row['product']
    ...
    goal_options = {
        'medical': {
            'gtx': [
                f"Leverage {product} (GTX series) for high-performance diagnostics and real-time imaging analysis.",
                f"Enhance secure data visualization and AI-driven insights with {product} graphics capabilities.",
                f"Optimize surgical planning simulations using {product}'s scalable computing power.",
                f"Streamline telemedicine rendering and patient portal interactions via {product}.",
                f"Improve compliance dashboards with {product}'s efficient data processing."
            ],
            'mg': [
                f"Deploy {product} (MG series) to manage patient workflows and resource scheduling.",
                f"Centralize analytics for outcomes tracking and predictive care modeling with {product}.",
                f"Facilitate regulatory reporting and audit trails using {product}'s management tools.",
                f"Enhance team collaboration on treatment plans via {product}.",
                f"Automate inventory for medical supplies with {product}'s advanced tracking features."
            ],
            'gtk': [
                f"Scale enterprise-wide health data platforms with {product} (GTK 500) for ultra-high throughput.",
                f"Enable advanced genomic sequencing visualizations using {product}'s powerhouse specs.",
                f"Integrate {product} for hospital-wide AI model training and deployment.",
                f"Boost real-time monitoring systems with {product}'s enterprise-grade reliability.",
                f"Transform research data handling with {product}'s massive parallel processing."
            ],
            'default': [
        ...
        'retail': {
            'gtx': [
                f"Power point-of-sale displays and dynamic pricing engines with {product} (GTX series).",
                f"Enable high-res customer-facing kiosks and AR try-ons using {product}'s graphics.",
                f"Scale e-commerce backend rendering for personalized recommendations via {product}.",
                f"Optimize store analytics visualizations with {product}'s fast compute.",
                f"Drive in-store digital signage with {product}'s reliable performance."
            ],
            'mg': [
                f"Manage omnichannel inventory and supplier coordination using {product} (MG series).",
                f"Track sales trends and demand forecasting with {product}'s analytics suite.",
                f"Automate merchandising workflows and planogram updates via {product}.",
                f"Centralize customer loyalty data management through {product}.",
                f"Streamline returns processing and fraud alerts with {product}."
            ],
            'gtk': [
                f"Handle massive retail data lakes and real-time analytics with {product} (GTK 500).",
                f"Deploy {product} for supply chain simulations and predictive logistics.",
                f"Empower enterprise CRM with {product}'s high-capacity processing.",
                f"Integrate {product} for global store network monitoring.",
                f"Accelerate big data queries on sales histories using {product}."
            ],
            'default': [
        ...
        'software': {
            'gtx': [
                f"Accelerate GPU-accelerated builds and simulations with {product} (GTX series).",
                f"Enhance 3D modeling and VR dev environments using {product}.",
                f"Scale ML training pipelines via {product}'s parallel processing.",
                f"Optimize game engine rendering with {product}.",
                f"Boost CI/CD visualization dashboards through {product}."
            ],
            'mg': [
                f"Orchestrate dev team workflows and issue tracking with {product} (MG series).",
                f"Manage release cycles and dependency graphs using {product}.",
                f"Automate code review processes via {product}.",
                f"Centralize sprint planning and backlog grooming through {product}.",
                f"Track security vulnerabilities with {product}'s monitoring tools."
            ],
            'gtk': [
                f"Power enterprise-scale simulations with {product} (GTK 500).",
                f"Deploy {product} for distributed computing clusters.",
                f"Enable high-fidelity data viz in dev tools using {product}.",
                f"Integrate {product} for AI-assisted coding at scale.",
                f"Handle petabyte-scale repo management via {product}."
            ],
            'default': [
        ...
        'services': {
            'gtx': [
                f"Visualize client project timelines and Gantt charts with {product} (GTX series) for immersive reviews.",
                f"Enhance virtual collaboration sessions using {product}'s high-res graphics rendering.",
                f"Scale resource allocation simulations via {product}'s compute power.",
                f"Optimize service blueprint visualizations with {product}.",
                f"Drive interactive training modules through {product}."
            ],
            'mg': [
                f"Coordinate client engagements and milestone tracking using {product} (MG series).",
                f"Manage contract renewals and SLA monitoring with {product}.",
                f"Automate feedback loops and satisfaction surveys via {product}.",
                f"Centralize knowledge base for service delivery through {product}.",
                f"Streamline billing cycles and invoice disputes with {product}."
            ],
            'gtk': [
                f"Handle enterprise service desk operations with {product} (GTK 500) for high-volume ticketing.",
                f"Deploy {product} for predictive maintenance in service fleets.",
                f"Empower global team dashboards with {product}'s robust processing.",
                f"Integrate {product} for cross-region compliance tracking.",
                f"Accelerate analytics on service metrics using {product}."
            ],
            'default': [
        ...
        'entertainment': {
            'gtx': [
                f"Render high-fidelity CGI scenes and VFX previews with {product} (GTX series).",
                f"Enable real-time motion capture processing using {product}'s graphics acceleration.",
                f"Scale audience analytics visualizations via {product}.",
                f"Optimize streaming bitrate adaptation with {product}.",
                f"Drive interactive fan experiences through {product}."
            ],
            'mg': [
                f"Orchestrate content production schedules and asset libraries using {product} (MG series).",
                f"Manage rights clearances and distribution rights with {product}.",
                f"Automate subtitle synchronization and localization via {product}.",
                f"Centralize talent booking and contract management through {product}.",
                f"Track viewer engagement metrics with {product}."
            ],
            'gtk': [
                f"Power massive content delivery networks with {product} (GTK 500).",
                f"Deploy {product} for live event streaming at scale.",
                f"Enable ultra-HD post-production pipelines using {product}.",
                f"Integrate {product} for global distribution analytics.",
                f"Handle petabyte-scale media archives via {product}."
            ],
            'default': [
        ...
        'telecommunications': {
            'gtx': [
                f"Visualize network topologies and traffic flows with {product} (GTX series) for troubleshooting.",
                f"Enhance spectrum analysis tools using {product}'s high-res rendering.",
                f"Scale simulation of 5G beamforming via {product}.",
                f"Optimize fault localization maps with {product}.",
                f"Drive VR network training modules through {product}."
            ],
            'mg': [
                f"Manage subscriber provisioning and billing cycles using {product} (MG series).",
                f"Orchestrate tower maintenance schedules with {product}.",
                f"Automate outage response workflows via {product}.",
                f"Centralize customer care queues through {product}.",
                f"Track network utilization trends with {product}."
            ],
            'gtk': [
                f"Process terabit-scale traffic logs with {product} (GTK 500).",
                f"Deploy {product} for predictive network capacity planning.",
                f"Empower core router monitoring with {product}'s power.",
                f"Integrate {product} for edge device orchestration.",
                f"Accelerate anomaly detection queries using {product}."
            ],
            'default': [
        ...
        'finance': {
            'gtx': [
                f"Render real-time market heatmaps and trading floors with {product} (GTX series).",
                f"Enhance algorithmic visualization dashboards using {product}.",
                f"Scale risk scenario simulations via {product}.",
                f"Optimize portfolio performance graphs with {product}.",
                f"Drive immersive compliance training through {product}."
            ],
            'mg': [
                f"Manage transaction reconciliation and ledger balancing using {product} (MG series).",
                f"Orchestrate compliance audits and reporting with {product}.",
                f"Automate fraud alert workflows via {product}.",
                f"Centralize client account management through {product}.",
                f"Track investment performance metrics with {product}."
            ],
            'gtk': [
                f"Handle high-frequency trading data streams with {product} (GTK 500).",
                f"Deploy {product} for global market surveillance.",
                f"Empower quantitative modeling with {product}'s capacity.",
                f"Integrate {product} for blockchain transaction validation.",
                f"Accelerate stress testing computations using {product}."
            ],
            'default': [
        ...
        'employment': {
            'gtx': [
                f"Visualize organizational charts and talent maps with {product} (GTX series) for strategic planning.",
                f"Enhance diversity analytics dashboards using {product}.",
                f"Scale employee engagement simulations via {product}.",
                f"Optimize career path visualizations with {product}.",
                f"Drive VR onboarding experiences through {product}."
            ],
            'mg': [
                f"Manage recruitment pipelines and candidate sourcing using {product} (MG series).",
                f"Orchestrate performance review cycles with {product}.",
                f"Automate benefits enrollment workflows via {product}.",
                f"Centralize learning module tracking through {product}.",
                f"Track turnover prediction models with {product}."
            ],
            'gtk': [
                f"Process enterprise HR data warehouses with {product} (GTK 500).",
                f"Deploy {product} for predictive hiring analytics.",
                f"Empower global workforce planning with {product}.",
                f"Integrate {product} for compliance reporting at scale.",
                f"Accelerate skills inventory queries using {product}."
            ],
            'default': [
        ...
        'marketing': {
            'gtx': [
                f"Render campaign performance heatmaps and A/B test visuals with {product} (GTX series).",
                f"Enhance ad creative previews using {product}'s graphics.",
                f"Scale audience journey simulations via {product}.",
                f"Optimize content mockup rendering with {product}.",
                f"Drive interactive brand experience demos through {product}."
            ],
            'mg': [
                f"Manage multi-channel campaign orchestration using {product} (MG series).",
                f"Track lead nurturing sequences with {product}.",
                f"Automate personalization rule sets via {product}.",
                f"Centralize asset libraries for content through {product}.",
                f"Monitor sentiment analysis feeds with {product}."
            ],
            'gtk': [
                f"Handle big data customer insights with {product} (GTK 500).",
                f"Deploy {product} for real-time personalization engines.",
                f"Empower global campaign analytics with {product}.",
                f"Integrate {product} for cross-platform attribution.",
                f"Accelerate predictive modeling on consumer behavior using {product}."
            ],
            'default': [
        ...
        'technolgy': {
            'gtx': [
                f"Accelerate prototype rendering and CAD models with {product} (GTX series).",
                f"Enhance IoT device simulations using {product}.",
                f"Scale AR/VR testing environments via {product}.",
                f"Optimize circuit board visualizations with {product}.",
                f"Drive hardware debugging interfaces through {product}."
            ],
            'mg': [
                f"Orchestrate R&D project pipelines using {product} (MG series).",
                f"Manage patent tracking and IP portfolios with {product}.",
                f"Automate testing suite deployments via {product}.",
                f"Centralize innovation backlog through {product}.",
                f"Track tech stack compatibility with {product}."
            ],
            'gtk': [
                f"Power supercomputing clusters for tech research with {product} (GTK 500).",
                f"Deploy {product} for quantum simulation frameworks.",
                f"Enable high-throughput data processing using {product}.",
                f"Integrate {product} for edge AI deployments.",
                f"Handle exascale modeling via {product}."
            ],
            'default': [
        ...
        'other': {
            'gtx': [
                f"Visualize operational dashboards and process flows with {product} (GTX series).",
                f"Enhance simulation tools for business scenarios using {product}.",
                f"Scale reporting graphics via {product}.",
                f"Optimize workflow diagrams with {product}.",
                f"Drive executive presentations through {product}."
            ],
            'mg': [
                f"Manage cross-departmental projects using {product} (MG series).",
                f"Orchestrate vendor collaborations with {product}.",
                f"Automate routine approvals via {product}.",
                f"Centralize document repositories through {product}.",
                f"Track KPI progress with {product}."
            ],
            'gtk': [
                f"Process enterprise-wide datasets with {product} (GTK 500).",
                f"Deploy {product} for strategic forecasting models.",
                f"Empower C-suite analytics with {product}.",
                f"Integrate {product} for legacy system migrations.",
                f"Accelerate decision support queries using {product}."
            ],
            'default': [
    ...
    sector_goals = goal_options.get(sector, {'default': goal_options['other']})
    series_goals = sector_goals.get(series, sector_goals.get('default', []))
    customer_goal = random.choice(series_goals) if series_goals else f"Discuss high-level goals with {product}."
    
    # For mid_activities
    sector_mids = mid_activities_options.get(sector, {'default': []})
    series_mids = sector_mids.get(series, sector_mids.get('default', []))
    mid_activities = [act.format(product=product) for act in series_mids]
    
    # For outcomes
    sector_won = won_outcome_options.get(sector, {'default': []})
    series_won = sector_won.get(series, sector_won.get('default', []))
    won_reasons = [reason.format(product=product) for reason in series_won]
    
    sector_lost = lost_outcome_options.get(sector, {'default': []})
    series_lost = sector_lost.get(series, sector_lost.get('default', []))
    lost_reasons = [reason.format(product=product) for reason in series_lost]
    ...
    for i in range(1, num_activities - 1):
        activity = random.choice(mid_activities) if mid_activities else "Follow-up activity conducted."
        notes.append(f"{activity_dates[i].strftime('%d-%m-%Y')}: {activity}")
    
    # Outcome
    if stage == 'Won':
        outcome_reason = random.choice(won_reasons) if won_reasons else "Closed won after successful negotiations."
        notes.append(f"{activity_dates[-1].strftime('%d-%m-%Y')}: {outcome_reason}")
    else:
        outcome_reason = random.choice(lost_reasons) if lost_reasons else "Deal lost due to unresolved objections."
        notes.append(f"{activity_dates[-1].strftime('%d-%m-%Y')}: {outcome_reason}")
