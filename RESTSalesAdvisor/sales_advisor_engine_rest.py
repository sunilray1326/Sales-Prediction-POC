...
class SalesAdvisorEngine:
    ...
    def __init__(self, log_level=logging.INFO):
        ...
        # Setup REST API endpoints and versions
        self.openai_api_version = "2024-12-01-preview"
        self.search_api_version = "2023-11-01"

        # Build base URLs for REST API calls
        self.openai_base_url = self.config['OPEN_AI_ENDPOINT'].rstrip('/')
        self.search_base_url = self.config['SEARCH_ENDPOINT'].rstrip('/')

        self.logger.info(f"Azure OpenAI REST API configured (version: {self.openai_api_version})")
        self.logger.info(f"Azure Cognitive Search REST API configured (version: {self.search_api_version})")
    ...
    def _embed_text(self, text):
        ...
        self.logger.info("=" * 80)
        self.logger.info("AZURE OPENAI EMBEDDING REQUEST (REST API)")
        self.logger.info("=" * 80)
        ...
        try:
            ...
            # Log response
            self.logger.info("-" * 80)
            self.logger.info("AZURE OPENAI EMBEDDING RESPONSE (REST API)")
            self.logger.info("-" * 80)
            self.logger.info(f"  Embedding Dimension: {len(embedding)}")
            self.logger.info(f"  Model: {result.get('model', 'N/A')}")

            # Log token usage if available
            if 'usage' in result:
                self.logger.info(f"  Token Usage: {result['usage'].get('total_tokens', 'N/A')}")
    ...
    def _get_top_matches(self, prompt, stage_filter, top_k=10):
        ...
        self.logger.info("=" * 80)
        self.logger.info("AZURE COGNITIVE SEARCH REQUEST (REST API)")
        self.logger.info("=" * 80)
        ...
        try:
            ...
            # Log response
            self.logger.info("-" * 80)
            self.logger.info("AZURE COGNITIVE SEARCH RESPONSE (REST API)")
            self.logger.info("-" * 80)
    ...
    def _extract_attributes(self, prompt):
        """Use LLM to extract key attributes from the user prompt using REST API."""
        self.logger.debug("Calling Azure OpenAI for attribute extraction (REST API)")
        ...
        try:
            ...
            self.logger.debug(f"OpenAI Chat Completion request (REST API):")
    ...
    def _llm_chat(self, messages, temperature=0.8, seed=None):
        ...
        self.logger.info("=" * 80)
        self.logger.info("AZURE OPENAI CHAT COMPLETION REQUEST (REST API)")
        self.logger.info("=" * 80)
        ...
        try:
            ...
            # Log full request parameters
            self.logger.info("REQUEST PARAMETERS:")
            self.logger.info(f"  Model: {self.config['CHAT_MODEL']}")
            ...
            # Log full response
            self.logger.info("-" * 80)
            self.logger.info("AZURE OPENAI CHAT COMPLETION RESPONSE (REST API)")
            self.logger.info("-" * 80)
            ...
            if 'model' in result:
                self.logger.info(f"  Model: {result['model']}")
